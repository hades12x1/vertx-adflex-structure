...
public class ObjectResultsExtractor {
    ...
    private void handleTopHitsAggregation(List<String> headers, List<Object> line, TopHits topHitsAggregation) {
        String name = topHitsAggregation.getName();
        if (!headers.contains(name)) {
            headers.add(name);
        }
        List<Map<String, Object>> docsAsMap = new ArrayList<>();
        for (SearchHit hit : topHitsAggregation.getHits()) {
    ...
    private void handleAggregations(Aggregations aggregations, List<String> headers, List<List<Object>> lines) {
        ...
        for (Aggregation aggregation : aggregationList) {
            ...
            if (aggregation instanceof TopHits) {
                ...
                handleTopHitsAggregation(headers, lines.get(currentLineIndex), topHitsAggregation);
                continue;
            }
    ...
    private void handleNumericMetricAggregation(List<String> header, List<Object> line, Aggregation aggregation) {
        ...
        //todo:Numeric MultiValue - Stats,ExtendedStats,Percentile...
            ...
            if (aggregation instanceof Stats) {
                ...
                throw new RuntimeException("unknown NumericMetricsAggregation.MultiValue:" + aggregation.getClass());
            }

        } else {
            throw new RuntimeException("unknown NumericMetricsAggregation" + aggregation.getClass());
        }
    }
